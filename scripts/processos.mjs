...
export function acompanhar(
  ...
  tinta,
  filho,
  { fatal = true, onClose, silencioso = false, semPrefixo = false } = {},
) {
  ...
  for (const fluxo of [filho.stdout, filho.stderr]) {
    ...
    fluxo.on('data', (pedaco) => {
      ...
      for (const linha of linhas) {
        process.stdout.write(`${semPrefixo ? '' : marca}${linha.replace(/\r$/, '')}\n`);
      }
    });
  }
...
/** Encerra somente um filho conhecido, sem derrubar os demais. */
export function encerrarFilho(filho) {
  if (!filho) return;
  matar(filho);
}
