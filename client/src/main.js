...
window.addEventListener('error', (e) => {
  reportLog('error', e.message, { filename: e.filename, lineno: e.lineno, colno: e.colno });
});
window.addEventListener('unhandledrejection', (e) => {
  reportLog('error', 'Unhandled Promise Rejection', { reason: String(e.reason) });
});
...
const PERFIS_QUALIDADE = {
  ...
  nitido: { bitrate: 5_000_000, fps: 60 },
  maximo: { bitrate: 8_000_000, fps: 60 },
};
...
function perfilAtual() {
  return (
    Object.entries(PERFIS_QUALIDADE).find(
      ([, perfil]) => perfil.bitrate === ajustes.bitrate && perfil.fps === ajustes.fps,
    )?.[0] ?? 'equilibrado'
  );
}
      ...
      if (myBroadcast) {
        ...
        try {
          ...
          if (err.name !== 'NotAllowedError') {
            toast(`Erro ao recuperar tela: ${err.message}`, true);
          }
          $('recoveryOverlay').hidden = false;
        }
      }
